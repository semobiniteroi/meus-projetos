...
const normalizeStr = str => str ? str.toString().normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase() : "";
...
window.mesclarPDFs = () => processarMesclagem('input-pdfs');
...
window.buscarContribuintesAdmin = () => {
    const termo = normalizeStr(_val('busca-contribuinte-admin')), tb = _el('lista-contribuintes');
    if (termo.length < 3) return tb.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">Digite pelo menos 3 caracteres.</td></tr>';
    tb.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">Buscando...</td></tr>';
    getDB().collection("requerimentos_exumacao").orderBy("data_registro", "desc").limit(1000).get().then(snap => {
        window.contribuintesMap = {}; const tL = termo.replace(/[\.\-\/\(\)\s]/g, '');
        snap.forEach(doc => {
            let d = doc.data(); if (!d.cpf) return;
            let sb = normalizeStr(`${d.resp_nome || ''} ${d.cpf || ''} ${d.rg || ''} ${d.telefone || ''}`), sp = sb.replace(/[\.\-\/\(\)\s]/g, '');
            if (sb.includes(termo) || (tL && sp.includes(tL))) window.contribuintesMap[d.cpf] = window.contribuintesMap[d.cpf] || d;
        });
        const lista = Object.values(window.contribuintesMap); tb.innerHTML = '';
        if (!lista.length) return tb.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">Nenhum encontrado.</td></tr>';
        lista.forEach(i => tb.innerHTML += `<tr><td><b style="color:#333;">${i.resp_nome?.toUpperCase()||'-'}</b></td><td><span style="color:#3699ff; font-weight:bold;">${i.cpf||'-'}</span><br><span style="font-size:11px; color:#666;"><b>RG:</b> ${i.rg||'-'}</span></td><td>${i.telefone||'-'}</td><td style="font-size:11px; color:#555;">${i.endereco||''}, ${i.numero||'S/N'} - ${i.bairro||''}</td><td style="text-align:right;"><button class="btn-action-square btn-action-edit" onclick="window.editarContribuinteAdmin('${i.cpf}')" title="Editar">✏️</button></td></tr>`);
    }).catch(e => tb.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:red;">Erro na busca.</td></tr>');
}
...
window.realizarBusca = () => {
    const termo = normalizeStr(_val('input-busca')); if(!termo) { if(!_val('filtro-data')) _setVal('filtro-data', pegarDataISO()); return carregarTabela(); }
    _setVal('filtro-data', ''); if(unsubscribe) unsubscribe(); const tl = termo.replace(/[\.\-\/\(\)\s]/g, '');
    unsubscribe = getDB().collection("requerimentos_exumacao").orderBy("data_registro", "desc").limit(1000).onSnapshot(snap => {
        ...
        snap.forEach(doc => {
            ...
            let sb = normalizeStr(`${d.protocolo||''} ${d.resp_nome||''} ${d.nome_falecido||''} ${d.cpf||''} ${d.rg||''} ${d.telefone||''} ${d.processo||''} ${d.cemiterio||''}`);
            let idx = 2; while(d['telefone'+idx]) { sb += normalizeStr(d['telefone'+idx]); idx++; }
            if (sb.includes(termo) || (tl && sb.replace(/[\.\-\/\(\)\s]/g, '').includes(tl))) lista.push(d);
...
function renderizarTabela(lista) {
    ...
    lista.forEach(i => {
        ...
        let servicosHtml = i.servico_requerido ? i.servico_requerido.split(',').map(s => `<span class="badge badge-blue">${s.trim().toUpperCase()}</span>`).join('') : '-';
        let cemBadge = `<span class="cemiterio-tag">📍 ${i.cemiterio==='OUTRO'?i.cemiterio_outro?.toUpperCase():i.cemiterio?.toUpperCase()}</span>`;
        
        tb.innerHTML += `<tr onclick="window.visualizarDocumentos('${i.id}')">
            <td style="vertical-align:middle;"><b>${i.resp_nome?.toUpperCase()||'-'}</b><br><div class="text-icon-sm">📞 ${tl||'-'}</div></td>
            <td style="vertical-align:middle;"><div class="text-icon">👤 <b>${i.nome_falecido?.toUpperCase()||'-'}</b></div></td>
...
window.imprimirDeclaracao = () => {
    ...
    if (tv === "capacidade_nicho") txtI = `Eu, ${b(upper(d.resp_nome))}, inscrito(a) sob o CPF n° ${b(cpf)}, `;
    else if (tv === "declaracao_residencia" || tv === "cancelamento_processo") txtI = `Eu, ${b(upper(d.resp_nome))}, ${tv==="declaracao_residencia"?'carteira de identidade':'portador(a) da carteira de identidade'} nº ${b(rg)}, inscrito(a) sob o CPF nº ${b(cpf)}.<br><br>`;
